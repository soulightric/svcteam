...
export default function NotificationBell() {
  ...
  const dropdownContent = open ? (
    <div
      className="fixed right-5 top-20 w-80 rounded-2xl shadow-2xl overflow-hidden z-[99999] animate-fade-up"
      style={{
        backgroundColor: "white",
        border: "1px solid #e2e8f0",
        boxShadow: "0 20px 60px rgba(15,27,45,0.15)",
      }}
      onClick={(e) => e.stopPropagation()} // cegah close saat klik di dalam
    >
      {/* Header */}
      <div className="px-4 py-3 flex items-center justify-between" style={{ backgroundColor: "#0f1b2d" }}>
        <div className="flex items-center gap-2">
          <Bell size={14} className="text-teal-400" />
          <span className="text-sm font-semibold text-white serif">Notifikasi</span>
          {unreadCount > 0 && (
            <span className="px-1.5 py-0.5 rounded-full text-[10px] font-bold" style={{ backgroundColor: "#ef4444", color: "white" }}>
              {unreadCount} baru
            </span>
          )}
        </div>
      ...
      {/* Footer */}
      {notifs.length > 0 && (
        <div className="px-4 py-2.5 border-t border-slate-100" style={{ backgroundColor: "#f8f7f4" }}>
          <button
            onClick={() => {
              setNotifs([]);
              const allIds = notifs.map((n) => n.id);
  ...
  return (
    <>
      ...
      {/* Render Portal hanya jika sudah mounted di client */}
